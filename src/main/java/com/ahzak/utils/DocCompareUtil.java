...
public class DocCompareUtil {
     ...
     * 匹配html标签
     */
    private static final Pattern HTML_TAG_PATTERN = Pattern.compile("<(?:(?:/?[A-Za-z]\\w*\\b(?:[=\\s](['\"]?)[\\s\\S]*?\\1)*)|(?:!--[\\s\\S]*?--))/?>");

    private DocCompareUtil() {
    }
     ...
     *
     * @param docTxt 文本
     * @return java.util.List<com.yuanzhiwei.textcompare.Line>
     * @author Zhu Kaixiao
     * @date 2019/7/20 17:41
     **/
    private static List<Line> spLine(String docTxt) {
        List<Line> lines;
        Matcher matcher = HTML_TAG_PATTERN.matcher(docTxt);

        if (matcher.find()) {
            lines = spRichTxt(docTxt);
        } else {
            lines = spPlainTxt(docTxt);
        }
    ...
    /**
     * 按行切分富文本
     * 该方法是以p标签来区分行, 每个内容不为空的p标签都认为是一行
     *
     * @param richTxt 富文本
     * @return java.util.List<com.jeecms.common.util.DocCompareUtil.Line>
     * @author Zhu Kaixiao
     * @date 2019/7/22 14:40
     **/
    private static List<Line> spRichTxt(String richTxt) {
        ...
        for (Element p : elements) {
            String content = Html2Text.getContent(p.outerHtml());
            Line line = new Line();
            if (StringUtils.isBlank(content)) {
                content = fetchImgDom(p.html());
                if (StringUtils.isBlank(content)) {
                    continue;
                } else {
                    line.setType(LineType.IMAGE);
                }
            } else {
     ...
     * @date 2019/7/22 14:40
     **/
    private static List<Line> spPlainTxt(String plainTxt) {
        AtomicInteger rowNo = new AtomicInteger();
        List<Line> lines = Arrays.stream(plainTxt.split("\n")).map(s -> {
            Line line = new Line();
            line.setType(LineType.TEXT);
            line.setRowNo(rowNo.incrementAndGet());
            line.setStr(s);
            return line;
     ...
     **/
    private static String fetchImgLink(String htmlDom) {
        Matcher imgMatch = IMG_PATTERN.matcher(htmlDom);
        if (imgMatch.find()) {
     ...
     **/
    private static String fetchImgDom(String htmlDom) {
        Document html = Jsoup.parse(htmlDom);
        Elements imgs = html.select("img");
        if (imgs.size() > 0) {
            String imgHtml = imgs.get(0).outerHtml();
            return imgHtml;
        }
        return "";
    }

    private static List<Map<String, Object>> compToLines(List<Line> lines, List<CompareLine> compareLines, boolean isLeft) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            rt.add(m);
            m.put("type", line.getType().getValue());
            m.put("rowNo", line.getRowNo());
            m.put("line", line.getStr());
            String incomplete = "";
            String prevStr = null;
            for (int j = i; j < compareLines.size(); j++) {
                CompareLine compareLine = compareLines.get(j);
                String lineText = isLeft ? compareLine.getLeft() : compareLine.getRight();
                if (lineText == null && prevStr != null) {
                    incomplete = prevStr;
                } else if (lineText != null && prevStr == null) {
                    incomplete += lineText;
                } else {
                    incomplete = "";
                }
                if (line.getStr().equals(lineText) || line.getStr().equals(incomplete)) {
                    m.put("modified", compareLine.modifyDesc());
                    break;
                }
                prevStr = lineText;
            }
            if (!m.containsKey("modified")) {
                m.put("modified", isLeft ? CompareLine.DELETE : CompareLine.ADD);
            }
        }
     ...
     * @date 2019/7/22 9:26
     **/
    public static Map<String, Object> textCompare(String left, String right) {
        Map<String, Object> ret = new HashMap<>(4);
        List<Line> t1 = spLine(left);
        List<Line> t2 = spLine(right);

        DiffMatchPatch dmp = new DiffMatchPatch();
        LinkedList<DiffMatchPatch.Diff> diffs = dmp.diff_main(
                StringUtils.join(t1.stream().map(Line::getStr).collect(Collectors.toList()), "\n"),
                StringUtils.join(t2.stream().map(Line::getStr).collect(Collectors.toList()), "\n"),
                true);
        List<CompareLine> compareLines = dmp.diffLines(diffs);
        List<Map<String, Object>> maps1 = compToLines(t1, compareLines, true);
        List<Map<String, Object>> maps2 = compToLines(t2, compareLines, false);
        ret.put("list1", maps1);
        ret.put("list2", maps2);
        return ret;
    }

    public static Map<String, Object> textCompare(File leftFile, File rightFile) {
        String leftRich = readText(leftFile);
        String rightRick = readText(rightFile);
        Map<String, Object> map = textCompare(leftRich, rightRick);
        return map;
    }


    public static void main(String[] args) throws URISyntaxException {
        File file1 = new File(DocCompareUtil.class.getClassLoader().getResource("1.html").toURI());
        File file2 = new File(DocCompareUtil.class.getClassLoader().getResource("2.html").toURI());

        Map<String, Object> map = textCompare(file1, file2);
     ...
     *
     * @param path 文件路径
     * @return java.lang.String 富文本字符串
     * @author Zhu Kaixiao
     * @date 2019/7/22 9:16
     **/
    ...
    private static String readText(File file) {
        ...
        try (FileReader r = new FileReader(file)) {
            ...
            int len;
            while ((len = r.read(chBuffer)) != -1) {
                sb.append(chBuffer, 0, len);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
     ...
     * @date 2019/7/22 9:15
     **/
    private static File writeHtml(String html) {
     ...
     */
    @Data
    private static class CompareLine {
     ...
     * behaviour settings.
     */
    private static class DiffMatchPatch {
        ...
        private static final Pattern patchHeader = Pattern.compile("^@@ -(\\d+),?(\\d*) \\+(\\d+),?(\\d*) @@$");
            ...
            for (Diff aDiff : diffs) {
                ...
                switch (aDiff.operation) {
                    case INSERT:
                        ...
                        break;
                    case DELETE:
                        html.append("<DEL STYLE=\"background:#FFE6E6;\" TITLE=\"i=").append(i).append("\">").append(text)
                                .append("</DEL>");
                        break;
                    case EQUAL:
        ...
        public List<CompareLine> diffLines(LinkedList<Diff> diffs) {
            List<CompareLine> ret = new ArrayList<>();
            ...
            for (Diff aDiff : diffs) {
                ...
                if (split.length == 0) {
                    compareLine.setLeft(Optional.ofNullable(lsb).map(StringBuilder::toString).orElse(null));
                    compareLine.setRight(Optional.ofNullable(rsb).map(StringBuilder::toString).orElse(null));
                ...
                switch (aDiff.operation) {
                    case INSERT:
                        ...
                        if (text.contains("\n")) {
                            compareLine.setLeft(Optional.ofNullable(lsb).map(StringBuilder::toString).orElse(null));
                            compareLine.setRight(rsb.toString());
                            compareLine = new CompareLine();
                            lsb = null;
                            rsb = null;
                            ...
                            if (split.length > 2) {
                                for (int i = 1; i < split.length - 1; i++) {
                                    ret.add(compareLine);
                                    compareLine.setLeft(null);
                                    compareLine.setRight(split[i]);
                                    compareLine = new CompareLine();
                                }
                            }
                            if (split.length > 1) {
                                if (text.endsWith("\n")) {
                                    compareLine.setRight(split[split.length - 1]);
                                    ret.add(compareLine);
                                    compareLine = new CompareLine();
                                    lsb = null;
                                    rsb = null;
                                } else {
                                    rsb = new StringBuilder(split[split.length - 1]);
                                }
                            }
                            ret.add(compareLine);
                        }
                    ...
                    case DELETE:
                        ...
                        lsb.append(split[0]);
                        if (text.contains("\n")) {
                            compareLine.setLeft(lsb.toString());
                            compareLine.setRight(Optional.ofNullable(rsb).map(StringBuilder::toString).orElse(null));
                            compareLine = new CompareLine();
                            lsb = null;
                            rsb = null;
                            if (split.length > 2) {
                                for (int i = 1; i < split.length - 1; i++) {
                                    ret.add(compareLine);
                                    compareLine.setLeft(split[i]);
                                    compareLine.setRight(null);
                                    compareLine = new CompareLine();
                                }
                            }
                            ...
                            if (split.length > 1) {
                                if (text.endsWith("\n")) {
                                    compareLine.setLeft(split[split.length - 1]);
                                    ret.add(compareLine);
                                    compareLine = new CompareLine();
                                    lsb = null;
                                    rsb = null;
                                } else {
                                    lsb = new StringBuilder(split[split.length - 1]);
                                }
                            }
                            ret.add(compareLine);
                        }
...
//		DiffMatchPatch dmp = new DiffMatchPatch();
//
//		String text1 = "<p>白日依山尽</p><p>欲穷千里目</p><p>Desert</p><p><span >黄河入海流</span><br/></p>";
//		String text2 = "<p>白日依山尽</p><p><span style=\"color: #D99694;\"><em><span style=\"text-decoration: none;\">王侯将相 宁有种乎?</span></em></span></p><p><span style=\"color: #974806; text-decoration: underline;\">黄河入海流</span><br/></p>";
//
//		// step1: 分行
//		List<Line> txt1 = spLine(text1);
//		List<Line> txt2 = spLine(text2);
//
//		LinkedList<DiffMatchPatch.Diff> diffs = dmp.diff_main(
//				StringUtils.join(txt1.stream().map(Line::getStr).collect(Collectors.toList()), "\n"),
//				StringUtils.join(txt2.stream().map(Line::getStr).collect(Collectors.toList()), "\n"),
//				true);
//		String html = dmp.diff_prettyHtml(diffs);
////		showHtml(writeHtml(html));
//	}
